...
with urllib.request.urlopen(req_dish, timeout=15) as response:
    ...
    for result in res.get('result', []):
        ...
        print(f"  Food: {name} (unicode: {unicode_escape}), probability: {prob}")
"""
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/test_baidu_image.py', 'w') as f:
        f.write(remote_py_code)
    sftp.close()
    
    print("=== Running Baidu Image Recognition Test on Remote ===")
    stdin, stdout, stderr = ssh.exec_command("python3 /tmp/test_baidu_image.py")
