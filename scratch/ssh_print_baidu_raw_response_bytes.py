...
with urllib.request.urlopen(req_dish, timeout=15) as response:
    ...
    print("Decoded UTF-8 string repr:", repr(raw_bytes.decode('utf-8', errors='replace')))
"""
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/test_baidu_bytes.py', 'w') as f:
        f.write(remote_py_code)
    sftp.close()
    
    print("=== Running Baidu Raw Bytes Test on Remote ===")
    stdin, stdout, stderr = ssh.exec_command("python3 /tmp/test_baidu_bytes.py")
